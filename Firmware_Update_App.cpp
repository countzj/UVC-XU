...
int xioctl(int request, void *arg)
{
  ...
  if (ret && (tries <= 0)) 
    printf("UVC: ioctl %d failed", request);
...
void reset()
{
    ...
    fprintf(stderr,"SREC upload failed. Board reset is advised before making another attempt.\n");
	buf_control1[0] = 0x00;
	buf_control1[1] = 0x00;
	buf_control1[2] = 0x00;
	buf_control1[3] = 0x00;
	...
	#ifdef LINUX
	if (!setControl(1,13,buf_control1))
	{
		fprintf(stderr,"Unable to reset system to normal state. This will require a system reset. Try update again after.\n");		
	}
	#endif
...
int main(int argc, char* argv[])
{
	...
	#ifdef LINUX
	if (!getControl(8, 88, buf_control8))
	{
		printf("Read Version Info\n");
	}
	else
	...
	while(bytes_left > 0){
		...
		#endif
		#ifdef LINUX
		if (!setControl(2,bytes_to_read,buffer))
		{
			reset();
			return 1;
		}
	...
	while(buf_control3[0]!= 0x01){
        ...
        #ifdef LINUX
		if (!getControl(3,2,buf_control3))
		{
			fprintf(stderr,"Unable to verify flash completion/failure. Board reset advised. Results of flashing unknown.\n");
			reset();
			return 1;
	...
	#ifdef LINUX
	if (!setControl(7,1,buf_control7))
	{
        fprintf(stderr,"Unable to automatically reset system. Manual reset advised. \n");
		return 1;
	}
