...
int main(int argc, char* argv[])
{
	...
	char path[128] = "/etc/cam/";
	srec = path;

	DIR           *pDir = NULL;
	struct dirent *ent;
	pDir = opendir("/etc/cam/");
	if (pDir == NULL)
	{
		printf("/etc/cam/ no exist\n");
		return 1;
	}
	...
	if (strcmp(srec, "/etc/cam/firm") <= 0){
		fprintf(stderr, "not found update file\n");
		return 1;
	}
	
	printf("have found update file %s\n", srec);
