...
int xioctl(int request, void *arg)
{
  ...
  if (ret) 
    LOGE("UVC%d: ioctl %d failed, try %d, error=%s\n",video_fd, request, tries, strerror(errno));
...
void reset()
{
	...
	{
		LOGE("Unable to reset system to normal state. This will require a system reset. Try update again after.\n");		
	}
...
int main(int argc, char* argv[])
{
	...
	while ((ent = readdir(pDir)) != NULL) {
		if (strstr(ent->d_name, "firm")) {
			srec = strcat(path, ent->d_name);
			break;
		} 
	}
	if (strcmp(srec, "/data/firm") <= 0){
		LOGE( "not found update file\n");
		return 1;
	}
	
	LOGE("have found update file %s\n", srec);
	
	uint8_t *buffer_srec = (uint8_t *) malloc(length_of_srec);
	if(buffer_srec==NULL){
		LOGE("Memory Error. Could not allocate buffer size\n");
		return 1;
	}
	...
	if(sizeof(bin_file_hdr)!=header.hdr_len){
		LOGE( "sizeof(header) does not equal header.hdr_len\n");
		goto fail1;
	}
    ...
    if (strcmp(signature, md5.digestString((char*)buffer_srec, data_length))!=0){
        LOGE( "Chosen firmware file does not have the correct md5 checksum signature.\n");
        goto fail1;
    }
    ...
    if (video_fd == -1)
    {
        ...
        LOGE("Opening Video device failed\n");
        goto fail1;
    }
	...
	if (getControl(XU_ID, 8, 88, buf_control8))
	{
		...
		LOGE("Read Version Info, version = %08x::%0x,%0x,%0x,%0x\n", version, buf_control8[0], buf_control8[1],buf_control8[2],buf_control8[3]);
	}
	else
	{
        LOGE("Can't Read Version Info\n");
    }
	...
	LOGE("Initiating firmware update.\n");
    if (!setControl(XU_ID, 1, 13, buf_control1))
    {
	    LOGE("Unable to initiate Firmware update. Please check to make sure video is not running.\n");
	    goto fail3;
    }
	
	LOGE( "System is ready to update. Sending binary data...\n");
	...
	LOGE("Sending data complete.\n");	
	LOGE( "Waiting for update completion...\n");	

	buf_control3[0] = 0x00;
	buf_control3[1] = 0x00;
	while(buf_control3[0]!= 0x01){
		if(buf_control3[0]==0x02){
			LOGE( "Flash write failure returned from SYS ARC! Try update again.\n");
			reset();
			goto fail3;
		}
		...
		{
			LOGE("Unable to verify flash completion/failure. Board reset advised. Results of flashing unknown.\n");
			reset();
			goto fail3;
		}
	}
	
	LOGE("Flash process completed.\n");
	LOGE("Finishing firmware update.\n");
    reset();
	
	LOGE("Resetting System...\n");

	buf_control7[0] = 0x01;

	if (!setControl(XU_ID, 7, 1, buf_control7))
	{
        LOGE("Unable to automatically reset system. Manual reset advised. \n");
	}
